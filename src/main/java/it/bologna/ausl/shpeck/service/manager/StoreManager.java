...
//            message.setAttachmentsNumber(EmlHandlerUtils.getAttachments(mailMessage.getOriginal(), null).length);
    ...
    public void updateMessageExtension(Message message,
        ...
        try {
            ...
            temp = (InternetAddress) from[0];
            if(temp.getAddress().contains("@")){ 
    ...
    public void storeMessagesAddresses(Message message, HashMap addresses) throws ShpeckServiceException {
        ...
        if (list != null && list.size() > 0) {
            ...
            for (Address address : list) {
                if(!address.getMailAddress().equals("MailSystemNotification")){
        ...
        if (from == null) {
            try {
                ...
                try {
                    ...
                    try {
                        log.error("unable to set From address from header", exe);
                        InternetAddress temp = new InternetAddress();
                        String header = mailMessage.getOriginal().getHeader("From", ","); 
                        String sottostringa = header.substring(0,header.lastIndexOf("<"));
                        temp.setAddress(sottostringa);
                        from[0] = temp;
                    } catch (Throwable e) {
    ...
    public HashMap upsertAddresses(MailMessage mailMessage) throws StoreManagerExeption, ShpeckServiceException {
        ...
        if (from != null) {
            InternetAddress temp = (InternetAddress) from[0]; 
            ...
            if(temp.getAddress().contains("@")) {
                ...
                address.setMailAddress("MailSystemNotification");
                address.setRecipientType(Address.RecipientType.UNKNOWN);
                fromArrayList = new ArrayList<Address>();
                fromArrayList.add(address);
                map.put("from", fromArrayList);
            }
        } else {
