...
public class MinIOPecStorage implements StorageStrategy {
    ...
    public UploadQueue storeMessage(Pec pec, String folderName, UploadQueue objectToUpload) throws ShpeckServiceException {
        ...
        try {
            ...
            // RM 63159: mettere idMessage e idPec negli additional data
            Map<String, Object> metaData = new HashMap<String, Object>();
            metaData.put("idPec", pec.getId());
            metaData.put("idMessage", objectToUpload.getIdRawMessage().getIdMessage().getId());

            MinIOWrapperFileInfo fileInfo = minIOWrapper.putWithBucket(new ByteArrayInputStream(baos.toByteArray()),
                    pec.getIdAziendaRepository().getCodice(),
                    path,
                    filename,
                    metaData, false, uuidMinIO, null);
