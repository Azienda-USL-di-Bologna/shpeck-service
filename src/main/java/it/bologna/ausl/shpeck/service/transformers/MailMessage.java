...
public class MailMessage implements MailIdentity {
    ...
    public MailMessage(MimeMessage m) throws MailMessageException {
        this.original = m;
        try {
            this.from = original.getHeader("From", ",") != null
                    ? InternetAddress.parseHeader(original.getHeader("From", ",")
                            .replaceAll("^\"+(.+?)\"+", "\"$1\"")
                            .replaceAll("<.*?'*?(.+?)'*?>", "<$1>") // toglie di apici tra le stringhe in <> es. <'ciao@ciao.it'> diventa <ciao@ciao.it>
                            .replaceAll(";\\s*>", ">") 
                            .replaceAll(";\\s*\">", "\"")
                            , true)
                    : null;
            this.to = original.getHeader("To", ",") != null
                    ? InternetAddress.parseHeader(original.getHeader("To", ",")
                            .replaceAll("^\"+(.+?)\"+", "\"$1\"")
                            .replaceAll("<.*?'*?(.+?)'*?>", "<$1>") // toglie di apici tra le stringhe in <> es. <'ciao@ciao.it'> diventa <ciao@ciao.it>
                            .replaceAll(";\\s*>", ">") 
                            .replaceAll(";\\s*\">", "\"")
                            , true)
                    : null;
            this.cc = original.getHeader("Cc", ",") != null
                    ? InternetAddress.parseHeader(original.getHeader("Cc", ",")
                            .replaceAll("^\"+(.+?)\"+", "\"$1\"")
                            .replaceAll("<.*?'*?(.+?)'*?>", "<$1>") // toglie di apici tra le stringhe in <> es. <'ciao@ciao.it'> diventa <ciao@ciao.it>
                            .replaceAll(";\\s*>", ">") 
                            .replaceAll(";\\s*\">", "\"")
    ...
    public MessageInterface.MessageType getType() throws ShpeckServiceException {
        return MessageInterface.MessageType.MAIL;
    }
